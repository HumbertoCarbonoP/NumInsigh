...
def NewtonCifrasSignificativas(func_str, func_deriv_str, x_inicial, tolerancia, max_iter):
    ...
    while error > tolerancia and abs(fx) > tolerancia and abs(fxPrima) > tolerancia and c < max_iter:
        ...
        fxPrima = valores_f_primera_derivada
        errores.append(abs((xn[-1] - xn[-2]) / xn[-1]))
        error = errores[-1]
        x_inicial = xn[-1]
        n.append(c + 1)
        c += 1
...
# Ejemplo de uso:
# resultado, n, xn, valores_f, errores = NewtonCifrasSignificativas(
#     "(math.pi**(-x)) * (-1 + x) + (x**(2/3)) - 79",
#     "-math.pi**(-x) * math.log(math.pi) * (-1 + x) + math.pi**(-x) + (2/3) * x**(-1/3)",
#     0, 5e-5, 100)
