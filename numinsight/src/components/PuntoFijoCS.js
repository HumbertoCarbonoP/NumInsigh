...
import PuntoFijoCSResult from './PuntoFijoCS-Result';

function PuntoFijoCS() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await fetch('http://localhost:5000/puntoFijoCS', {
                method: 'POST',
                headers: {
    ...
    return (
        <div>
            ...
            <PuntoFijoCSResult result={resultData} />
        </div>
    );
}

export default PuntoFijoCS;
