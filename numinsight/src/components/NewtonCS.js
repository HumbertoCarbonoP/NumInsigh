...
import NewtonCSResult from './NewtonCS-Result';
...
function NewtonCS() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await fetch('http://localhost:5000/newtonCS', {
                method: 'POST',
                headers: {
    ...
    return (
        <div>
            ...
            <NewtonCSResult result={resultData} />
        </div>
    );
}

export default NewtonCS;
